...
function doTheTreeViz(control,EWD) {
                ...
                setTimeout(function(){
                    if (control.nodeClickInProgress) {
                        ...
                        if (control.options.nodeFocus) {
                            ...
                            doTheTreeViz(makeFilteredData(control,EWD));
                        }
                    }
                },control.clickHack);
            }
...
function initializeConceptMap (EWD) {
    ...
    getTheData(control,EWD).then( function (data) {
...
function getTheData(control,EWD) {
    var dataPromise = getPromiseData(EWD);
...
function getPromiseData(EWD){
    ...
    EWD.send(messageObj, (res) => {
      if (res.message.output) {
        var data = res.message.output;
        deferred.resolve(data);
      }else{
        deferred.reject("error loading Concept Map");
      }
      $('#loadingModal').modal('hide');
    });
    /*$.getJSON('/ewd-vista/assets/javascripts/CMAP.json')
    .done( (data) => {
      //console.log(data);
      deferred.resolve(data);
    });*/
    /*$.getJSON('/ewd-vista/assets/javascripts/CMAP.json', null,
        function (data) {
            deferred.resolve(data);
        })
